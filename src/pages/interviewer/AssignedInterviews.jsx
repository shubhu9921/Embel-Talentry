...
const AssignedInterviews = () => {
    ...
    const stats = {
        ...
        today: interviews.filter(i =>
            (i.scheduledAt || '').startsWith(new Date().toISOString().split('T')[0])).length,
    ...
    return (
        <div className="space-y-10 page-fade-in">
            <div className="sticky top-0 z-10 bg-slate-50/90 backdrop-blur-md pb-4 pt-4 border-b border-slate-200/50 mb-4">
                ...
                <p className="text-slate-500 font-medium mt-1">Manage your upcoming technical rounds.</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {[
                    ...
                    <Card key={i} noPadding className="p-6 shadow-elevation-high ring-1 ring-slate-100 border-none">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-black text-[#19325c]">{String(s.value).padStart(2, '0')}</p>
                            </div>
                        </div>
                    </Card>
                ))}
            ...
            <div className="grid grid-cols-1 gap-4">
                {interviews.length > 0 ? interviews.map(interview => (
                    <Card key={interview.id}
                        className="p-6 border-none shadow-elevation-high ring-1 ring-slate-100 flex items-center justify-between group hover:ring-orange-100 transition-all">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-1">
                                    <h3 className="text-base font-black text-[#19325c]">{interview.candidateName}</h3>
                                ...
                                <div className="flex items-center gap-4 text-xs font-medium text-slate-400">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{new Date(interview.scheduledAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                        <Clock className="w-3.5 h-3.5" />
                                        <span>{new Date(interview.scheduledAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <Button variant="outline" className="rounded-xl flex items-center gap-2 border-slate-200">
                                <ExternalLink className="w-4 h-4" /><span>Join Round</span>
                            </Button>
                            <Button className="rounded-xl flex items-center gap-2">
                                <span>Fill Feedback</span><ArrowRight className="w-4 h-4" />
                            </Button>
                        </div>
                    </Card>
                )) : (
                    <div className="py-20 text-center">
                        ...
                        <p className="text-slate-400 font-medium max-w-xs mx-auto mt-2">You don't have any assigned interviews at the moment.</p>
                    </div>
                )}
            </div>
        </div>
